<script lang="ts">
  ...
  import { bootHistoryShellDemo } from "./create-shell-demo";
  ...
  onMount(() => {
    let cancelled = false;
    const runtimePromise = bootHistoryShellDemo();
    void runtimePromise
      .then((runtime) => {
        if (cancelled) return;
        app = runtime.app;
...
<div
  bind:this={root}
  class="history-shell-demo"
  data-testid="history-shell-demo"
  data-status={status}
>
  <output
    class="history-shell-demo__status"
    data-testid="history-shell-status"
  >
    {status}
  </output>
  {#if error}
...
<style>
  :global(
      body.sb-main-fullscreen:has(#storybook-root .history-shell-demo)
        #storybook-root
    ) {
  ...
  :global(html:has(#storybook-root .history-shell-demo)),
  :global(body:has(#storybook-root .history-shell-demo)) {
    overflow: hidden;
  }
  ...
  :global(.workspace-shell-docs-canvas) .history-shell-demo {
    height: 700px;
    max-height: 700px;
    min-height: 700px;
  }

  .history-shell-demo__status {
    position: absolute;
    width: 1px;
    height: 1px;
    overflow: hidden;
