...
if (!bundlePaths.length)
  throw new Error(`No payload ZIPs under ${releaseRoot}.`);

for (const bundlePath of bundlePaths) {
  const bundleBytes = await readFile(bundlePath);
  const releaseManifest = JSON.parse(
    await readFile(path.join(path.dirname(bundlePath), "release.json"), "utf8")
  );
  verifyPluginPayload({ bundleBytes, releaseManifest });
...
async function findBundles(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    else if (entry.isFile() && entry.name.endsWith(".payload.zip"))
      paths.push(target);
  }
  return paths.sort();
}
