...
async function captureAll(baseUrl) {
  ...
  for (const capture of captures) {
    ...
    const result = await captureSubjectWithBrowser(
      {
        ...
        deviceScaleFactor: 2,
        cropToViewport: true,
        globals: "colorMode:light;theme:lapis",
        browser: "chromium",
      },
      (progress) => {
        if (progress.phase !== phase) {
          ...
          process.stdout.write(
            `${options.check ? "Checking" : "Capturing"} ${capture.storyId}: ${progress.label}\n`,
          );
        }
      },
    );
    ...
    const composed = await composeRegistryMedia({
      source: Buffer.from(result.pngBase64, "base64"),
      focus: capture.focus,
      card: capture.card,
      fontPath,
    });
    for (const variant of ["preview", "full"]) {
      ...
      if (options.check) {
        ...
        if (!current || !current.equals(bytes)) mismatches.push(output.relativePath);
      } else {
  ...
  if (mismatches.length) {
    throw new Error(
      `Registry media is stale or missing:\n${mismatches.map((value) => `- ${value}`).join("\n")}\nRun pnpm registry:media:capture${options.plugin ? ` -- --plugin ${options.plugin}` : ""}.`,
    );
  }
  if (options.check) {
    process.stdout.write(
      `Verified ${captures.length} deterministic registry media cards.\n`,
    );
  }
}
  ...
  for (const plugin of selected) {
    ...
    for (const item of source.gallery ?? []) {
      ...
      captures.push({
        ...
        card: item.card,
        images: {
          preview: resolveOutput(
            packageRoot,
            item.images?.preview?.path,
            plugin.directory,
          ),
          full: resolveOutput(
            packageRoot,
            item.images?.full?.path,
            plugin.directory,
          ),
        },
      });
...
function resolveOutput(packageRoot, relativePath, directory) {
  if (
    typeof relativePath !== "string" ||
    !/^registry-assets\/gallery\/[a-z0-9][a-z0-9-]*\.(?:preview|full)\.webp$/.test(
      relativePath,
    )
  ) {
    throw new Error(`${directory}: unsafe registry media output ${relativePath}.`);
  }
...
function validateCatalogEntry(catalog, capture) {
  ...
  for (const tag of ["registry-media", "visual-pending"]) {
    if (!story.tags?.includes(tag)) {
      throw new Error(`${capture.pluginDirectory}: ${capture.storyId} requires tag ${tag}.`);
    }
  }
}
