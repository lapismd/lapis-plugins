...
for (const plugin of pluginPackages) {
  ...
  // Changesets owns CHANGELOG.md. Documentation synchronization must not
  // replace its version history with an extraction-era template.
  ...
  if (mode === "--write") {
    ...
    findings.push(
      `${path.relative(
        root,
        filePath
      )} is not synchronized; run pnpm packages:sync`
    );
  }
}
